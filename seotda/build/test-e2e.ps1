...
try {
    ...
    $script:base = 0
    $script:pcount = 2
    function PlayRound($label, $firstAct) {
        if ((Phase (St)) -eq '1' -and $firstAct) { $xl.Run($firstAct) }
        $guard = 0
        while ((Phase (St)) -eq '1' -and $guard -lt 40) { $xl.Run('OnCall'); $guard++ }
        $s = St
        $t = Total $s
        Write-Host ("{0:HH:mm:ss} {1} : state={2} total={3}" -f (Get-Date), $label, $s, $t)
        if ($t -ne $script:base) {
            # 파산 → 게임오버 → 자금 초기화가 발생하면 총자금이 정확히 10000×인원이 된다
            if ($t -eq (10000 * $script:pcount)) {
                Write-Host "  (파산 리셋 감지: 기준 $($script:base) -> $t)"
                $script:base = $t
            } else {
                throw "자금 보존 실패($label): $s (기준 $($script:base))"
            }
        }
    ...
    if ($script:base -ne 20000) { throw "2인 시작 총자금 오류: $($script:base)" }
    PlayRound 'A-R1(콜만)' $null
    $xl.Run('OnNextRound')
    PlayRound 'A-R2(하프)' 'OnHalf'
    $xl.Run('OnNextRound')
    PlayRound 'A-R3(다이)' 'OnDie'
    ...
    $s = St
    $t = Total $s
    Write-Host ("{0:HH:mm:ss} A-R4(따당) : state={1} total={2}" -f (Get-Date), $s, $t)
    if ($t -ne $script:base) {
        if ($t -eq (10000 * $script:pcount)) {
            Write-Host "  (파산 리셋 감지: 기준 $($script:base) -> $t)"
            $script:base = $t
        } else {
            throw "자금 보존 실패(A-R4): $s"
        }
    }
    ...
    PlayRound 'B-R1(다이 후 AI끼리 진행)' 'OnDie'
    $xl.Run('OnNextRound')
    PlayRound 'B-R2(하프)' 'OnHalf'
    for ($r = 3; $r -le 6; $r++) {
        $xl.Run('OnNextRound')
        PlayRound "B-R$r(콜)" $null
    }
    Write-Host 'E2E 통과'
    $wb.Close($false)
}
