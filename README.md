...
> ✅ **여기까지 하면 시트 게임 5종(오목·픽셀·라이어·미로·경마)은 전부 동작한다.**
> 게임판은 첫 방 생성 때 공장이 자동으로 그리므로 게임별 추가 설정이 없다.
> **퀴즈쇼만** 슬라이드 템플릿이 필요하다 → B-5.
...
### B-5. 퀴즈쇼 템플릿 등록 (슬라이드 게임 전용, 1회)

퀴즈쇼는 시트 게임과 달리 공장이 판을 그리지 않고 **스크립트가 설치된 템플릿
프레젠테이션을 복제**해서 방을 만든다. 그래서 템플릿 1개를 만들어 등록해야 한다.

1. **템플릿 만들기** — 새 프레젠테이션 → 확장 프로그램 → Apps Script
   - `Code.gs`에 `apps-script/slides-quiz.gs` 내용 붙여넣기
   - 파일 추가(+) → HTML → 이름을 정확히 **`slides-quiz-sidebar`** 로 만들고
     `apps-script/slides-quiz-sidebar.html` 내용 붙여넣기 → 저장
2. **보드 미리 그리기** — 프레젠테이션 새로고침 → 메뉴 **[🧠 퀴즈쇼] → [보드 생성]** 1회 실행
   (이걸 해두면 복제되는 모든 방이 열자마자 게임 가능한 상태가 된다)
3. **공장에 등록** — 공장 스크립트 편집기 → 프로젝트 설정(⚙️) → 스크립트 속성 추가:
   `QUIZ_TEMPLATE_ID` = 템플릿 파일 ID (URL의 `/d/`와 `/edit` 사이 문자열)

> ⚠️ 템플릿은 공장을 배포한 계정의 드라이브에 있어야 하고(복제 권한), 방마다 진행자가
> 복제본에서 처음 메뉴를 누를 때 권한 승인을 한 번 한다.

> 📗 게임별 사용법·트러블슈팅 등 운영 상세는 **[SETUP.md](SETUP.md)** 에 따로 정리돼 있다.
