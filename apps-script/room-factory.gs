...
const GAMES = {
  ...
  seotda: {
    ...
    maxPlayers: 6,
    roleLabel: i => '🎴 플레이어 ' + (i + 1),
    state: (roomId, n, opts) => ({ phase: 'idle', names: {}, chips: {}, bets: {}, hands: {}, pot: 0, n: n }),
    tplDraw: sheet => drawSeotdaBoard(sheet, 0, 6),
    patch: patchSeotdaFile,
    onEdit: handleSeotdaEdit,
  },
  maze: {
    name: '3D 미로 탈출',
    sheetName: '미로',
...
const POOL_TARGETS = {
  omok_v1: 4, maze_v1: 4, liar_v1: 8, horse_v1: 2, seotda_v1: 6,
...
// ---------- 섯다 (2~6인, 1인 1링크 — 내 패는 내 파일에만 보인다) ----------
// 족보 로직은 정빈의 엑셀 VBA(seotda/build/vba/modHand.bas)를 이식.
// 하우스룰: 구사 재경기 없음(라벨만 표기), 베팅은 1회전(콜 10 / 하프 = 팟 절반 / 다이).

const ST = {
  sheet: '섯다',
  banner: { row: 2, col: 2, width: 10 },
  card1: { row: 4, col: 3, rows: 5, cols: 2 },
  card2: { row: 4, col: 6, rows: 5, cols: 2 },
  jokbo: { row: 10, col: 3, width: 5 },
  potLabel: { row: 12, col: 2 }, pot: { row: 12, col: 3 },
  chipLabel: { row: 12, col: 5 }, chip: { row: 12, col: 6 },
  rosterRow: 15, rosterCol: 2,
  pcol: 10, // 우측 패널 열(J)
  nickLabel: { row: 4, col: 10 }, nick: { row: 5, col: 10 },
  startLabel: { row: 7, col: 10 }, start: { row: 8, col: 10 },
  betLabel: { row: 10, col: 10 }, bet: { row: 11, col: 10 },
  stateLabel: { row: 13, col: 10 }, state: { row: 14, col: 10 },
};
const ST_ANTE = 10;
const ST_START_CHIPS = 100;
const ST_CARD_BG = '#B71C1C';
const ST_CARD_FG = '#FFF8E1';

function stMonth(c) { return Math.floor(c / 10); }
function stIsGwang(c) { return c === 11 || c === 31 || c === 81; }
function stCardText(c) {
  return stMonth(c) + '월\n' + (stIsGwang(c) ? '🌕 광' : '🎴');
}

/** 정빈 modHand.bas HandRank 이식 — 클수록 강함 */
function stHandRank(c1, c2) {
  const m1 = stMonth(c1), m2 = stMonth(c2);
  const lo = Math.min(m1, m2), hi = Math.max(m1, m2);
  if (stIsGwang(c1) && stIsGwang(c2)) {
    if (lo === 3 && hi === 8) return 1000;
    if (lo === 1 && hi === 8) return 910;
    if (lo === 1 && hi === 3) return 905;
  }
  if (m1 === m2) return 800 + m1;
  if (lo === 1 && hi === 2) return 750;
  if (lo === 1 && hi === 4) return 740;
  if (lo === 1 && hi === 9) return 730;
  if (lo === 1 && hi === 10) return 720;
  if (lo === 4 && hi === 10) return 710;
  if (lo === 4 && hi === 6) return 700;
  return 100 + ((m1 + m2) % 10);
}

function stHandName(rank) {
  if (rank === 1000) return '38광땡';
  if (rank === 910) return '18광땡';
  if (rank === 905) return '13광땡';
  if (rank > 800 && rank <= 810) return rank === 810 ? '장땡' : (rank - 800) + '땡';
  if (rank === 750) return '알리';
  if (rank === 740) return '독사';
  if (rank === 730) return '구삥';
  if (rank === 720) return '장삥';
  if (rank === 710) return '장사';
  if (rank === 700) return '세륙';
  if (rank === 109) return '갑오';
  if (rank === 100) return '망통';
  return (rank - 100) + '끗';
}

function stIsDdaengJabi(c1, c2) {
  const m1 = stMonth(c1), m2 = stMonth(c2);
  return (m1 === 3 && m2 === 7) || (m1 === 7 && m2 === 3);
}
function stIsAmhaeng(c1, c2) {
  return (c1 === 41 && c2 === 71) || (c1 === 71 && c2 === 41);
}

function stHandLabel(c1, c2) {
  const base = stHandName(stHandRank(c1, c2));
  if (stIsAmhaeng(c1, c2)) return base + '·암행어사';
  if (stIsDdaengJabi(c1, c2)) return base + '·땡잡이';
  return base;
}

/** 쇼다운 시 유효 랭크 — 특수패는 상대 패를 보고 정해진다 */
function stEffectiveRank(myCards, othersRanks) {
  const base = stHandRank(myCards[0], myCards[1]);
  const bestOther = Math.max.apply(null, othersRanks.concat([0]));
  // 암행어사: 13·18광땡을 잡는다 (38광땡은 못 잡음)
  if (stIsAmhaeng(myCards[0], myCards[1]) && (bestOther === 905 || bestOther === 910)) return 950;
  // 땡잡이: 1땡~9땡을 잡는다 (장땡·광땡은 못 잡음)
  if (stIsDdaengJabi(myCards[0], myCards[1]) && bestOther > 800 && bestOther <= 809) return 815;
  return base;
}

function drawSeotdaBoard(sheet, idx, n) {
  sheet.getRange(ST.banner.row, ST.banner.col, 1, ST.banner.width).merge();
  sheet.setRowHeight(2, 46);
  sheet.getRange(ST.banner.row, ST.banner.col)
    .setValue('🎴 닉네임 입력 → 전원 모이면 아무나 [게임 시작] 체크!')
    .setFontWeight('bold').setFontSize(12)
    .setHorizontalAlignment('center').setVerticalAlignment('middle')
    .setWrap(true).setBackground('#FFEBEE');

  // 내 패 두 장 — 큰 카드 블록
  [ST.card1, ST.card2].forEach(cd => {
    sheet.getRange(cd.row, cd.col, cd.rows, cd.cols).merge();
    sheet.getRange(cd.row, cd.col)
      .setValue('🎴')
      .setFontSize(22).setFontWeight('bold').setWrap(true)
      .setHorizontalAlignment('center').setVerticalAlignment('middle')
      .setBackground(ST_CARD_BG).setFontColor(ST_CARD_FG);
  });
  sheet.setRowHeights(ST.card1.row, ST.card1.rows, 30);

  sheet.getRange(ST.jokbo.row, ST.jokbo.col, 1, ST.jokbo.width).merge();
  sheet.getRange(ST.jokbo.row, ST.jokbo.col)
    .setValue('내 족보: —').setFontWeight('bold').setFontSize(14)
    .setHorizontalAlignment('center');

  sheet.getRange(ST.potLabel.row, ST.potLabel.col).setValue('💰 판돈').setFontWeight('bold');
  sheet.getRange(ST.pot.row, ST.pot.col).setValue(0).setFontSize(14).setFontWeight('bold');
  sheet.getRange(ST.chipLabel.row, ST.chipLabel.col).setValue('🪙 내 칩').setFontWeight('bold');
  sheet.getRange(ST.chip.row, ST.chip.col).setValue(ST_START_CHIPS).setFontSize(14).setFontWeight('bold');

  sheet.getRange(ST.rosterRow - 1, ST.rosterCol).setValue('👥 참가자').setFontWeight('bold');
  for (let i = 0; i < n; i++) {
    sheet.getRange(ST.rosterRow + i, ST.rosterCol)
      .setValue((i + 1) + '. 플레이어' + (i + 1) + (i === idx ? '  ← 나' : ''));
  }

  const put = (pos, txt) => sheet.getRange(pos.row, pos.col)
    .setValue(txt).setFontWeight('bold');
  put(ST.nickLabel, '1️⃣ 내 닉네임 ↓');
  sheet.getRange(ST.nick.row, ST.nick.col).setBackground(NICK_YELLOW)
    .setBorder(true, true, true, true, false, false, '#F9A825', SpreadsheetApp.BorderStyle.SOLID_MEDIUM);
  put(ST.startLabel, '게임 시작 ↓');
  sheet.getRange(ST.start.row, ST.start.col).insertCheckboxes();
  put(ST.betLabel, '🎲 베팅 ↓ (콜/하프/다이)');
  sheet.getRange(ST.bet.row, ST.bet.col).setBackground('#E8F5E9')
    .setBorder(true, true, true, true, false, false, '#188038', SpreadsheetApp.BorderStyle.SOLID_MEDIUM);
  put(ST.stateLabel, '상태');
  sheet.getRange(ST.state.row, ST.state.col).setValue('대기 중');
  sheet.setColumnWidth(ST.pcol, 190);
  sheet.setColumnWidth(ST.rosterCol, 210);
  sheet.setHiddenGridlines(true);
}

function patchSeotdaFile(sheet, idx, room) {
  const n = room.fileIds ? room.fileIds.length : room.n;
  for (let i = 0; i < 6; i++) {
    sheet.getRange(ST.rosterRow + i, ST.rosterCol)
      .setValue(i < n ? (i + 1) + '. 플레이어' + (i + 1) + (i === idx ? '  ← 나' : '') : '');
  }
...
function stEachSheet(room, fn) {
  room.fileIds.forEach((id, i) =>
    fn(SpreadsheetApp.openById(id).getSheetByName(ST.sheet), i));
}

function stName(room, i) {
  return (room.names && room.names[i]) || '플레이어' + (i + 1);
}

function stSyncRoster(room) {
  const n = room.fileIds.length;
  stEachSheet(room, (s, me) => {
    for (let i = 0; i < n; i++) {
      const chips = room.chips[i] != null ? room.chips[i] : ST_START_CHIPS;
      const bet = room.phase === 'bet'
        ? (room.bets[i] ? '  · ' + room.bets[i] : '  · 고민 중…') : '';
      s.getRange(ST.rosterRow + i, ST.rosterCol)
        .setValue((i + 1) + '. ' + stName(room, i) + '  🪙' + chips + bet + (i === me ? '  ← 나' : ''));
    }
  });
}

function stDeal(room) {
  const n = room.fileIds.length;
  // 20장 화투: 1~10월 × 2장 (id = 월*10 + 1|2)
  const deck = [];
  for (let m = 1; m <= 10; m++) { deck.push(m * 10 + 1, m * 10 + 2); }
  for (let i = deck.length - 1; i > 0; i--) {
    const j = Math.floor(Math.random() * (i + 1));
    const t = deck[i]; deck[i] = deck[j]; deck[j] = t;
  }
  room.hands = {}; room.bets = {}; room.pot = 0;
  for (let i = 0; i < n; i++) {
    room.hands[i] = [deck.pop(), deck.pop()];
    if (room.chips[i] == null) room.chips[i] = ST_START_CHIPS;
    const ante = Math.min(ST_ANTE, room.chips[i]);
    room.chips[i] -= ante; room.pot += ante;
  }
  room.phase = 'bet';
  stEachSheet(room, (s, i) => {
    const h = room.hands[i];
    s.getRange(ST.card1.row, ST.card1.col).setValue(stCardText(h[0]));
    s.getRange(ST.card2.row, ST.card2.col).setValue(stCardText(h[1]));
    s.getRange(ST.jokbo.row, ST.jokbo.col).setValue('내 족보: ' + stHandLabel(h[0], h[1]));
    s.getRange(ST.pot.row, ST.pot.col).setValue(room.pot);
    s.getRange(ST.chip.row, ST.chip.col).setValue(room.chips[i]);
    s.getRange(ST.bet.row, ST.bet.col).setValue('');
    s.getRange(ST.state.row, ST.state.col).setValue('베팅하세요!');
    s.getRange(ST.banner.row, ST.banner.col)
      .setValue('🎲 패 확인! 오른쪽 베팅 칸에 콜 / 하프 / 다이 입력 (판돈 ' + room.pot + ')')
      .setFontColor('#000000').setBackground('#FFF9C4');
  });
  stSyncRoster(room);
}

function stShowdown(room) {
  const n = room.fileIds.length;
  const alive = [];
  for (let i = 0; i < n; i++) { if (room.bets[i] !== '다이') alive.push(i); }
  const baseRanks = {};
  alive.forEach(i => { baseRanks[i] = stHandRank(room.hands[i][0], room.hands[i][1]); });
  const eff = {};
  alive.forEach(i => {
    const others = alive.filter(j => j !== i).map(j => baseRanks[j]);
    eff[i] = stEffectiveRank(room.hands[i], others);
  });
  let winners = [];
  if (alive.length) {
    const best = Math.max.apply(null, alive.map(i => eff[i]));
    winners = alive.filter(i => eff[i] === best);
  }
  const share = winners.length ? Math.floor(room.pot / winners.length) : 0;
  winners.forEach(i => { room.chips[i] += share; });

  const lines = [];
  for (let i = 0; i < n; i++) {
    const h = room.hands[i];
    lines.push((i + 1) + '. ' + stName(room, i) + ': ' +
      (room.bets[i] === '다이' ? '💀 다이'
        : stMonth(h[0]) + '월+' + stMonth(h[1]) + '월 → ' + stHandLabel(h[0], h[1])));
  }
  const winText = winners.length
    ? '🏆 ' + winners.map(i => stName(room, i)).join(', ') + ' 님이 ' +
      stHandLabel(room.hands[winners[0]][0], room.hands[winners[0]][1]) +
      (winners.length > 1 ? ' (무승부 분배)' : '') + '(으)로 판돈 ' + room.pot + ' 획득!'
    : '전원 다이 — 판돈은 다음 판으로';
  if (!winners.length) { /* 전원 다이: 팟 이월 */ } else { room.pot = 0; }

  room.phase = 'idle';
  stEachSheet(room, (s, i) => {
    s.getRange(ST.banner.row, ST.banner.col).setValue(winText)
      .setBackground('#FFF176').setFontColor('#000000');
    s.getRange(ST.state.row, ST.state.col).setValue('라운드 종료 — [게임 시작]으로 다음 판');
    s.getRange(ST.chip.row, ST.chip.col).setValue(room.chips[i]);
    s.getRange(ST.pot.row, ST.pot.col).setValue(room.pot);
    // 전원 패 공개 — 명단 옆에
    for (let k = 0; k < n; k++) {
      s.getRange(ST.rosterRow + k, ST.rosterCol).setValue(lines[k] + (k === i ? '  ← 나' : ''));
    }
  });
}

function handleSeotdaEdit(e, room, roomId) {
  const srcIdx = room.fileIds.indexOf(e.source.getId());
  if (srcIdx < 0) return;
  const sheet = e.range.getSheet();
  if (sheet.getName() !== ST.sheet) return;
  const r = e.range.getRow(), col = e.range.getColumn();

  if (r === ST.nick.row && col === ST.nick.col) {
    room.names[srcIdx] = String(e.value || '').trim().slice(0, 12);
    stSyncRoster(room);
    return;
  }

  if (r === ST.start.row && col === ST.start.col && e.value === 'TRUE') {
    e.range.setValue(false);
    if (room.phase === 'bet') return; // 판 진행 중엔 무시
    stDeal(room);
    return;
  }

  if (r === ST.bet.row && col === ST.bet.col) {
    if (room.phase !== 'bet') { e.range.setValue(''); return; }
    if (room.bets[srcIdx]) { e.range.setValue(room.bets[srcIdx]); return; } // 번복 금지
    const v = String(e.value || '').trim();
    if (v !== '콜' && v !== '하프' && v !== '다이') {
      e.range.setValue('');
      sheet.getRange(ST.state.row, ST.state.col).setValue('⚠️ 콜/하프/다이만!');
      return;
    }
    let add = 0;
    if (v === '콜') add = ST_ANTE;
    if (v === '하프') add = Math.max(ST_ANTE, Math.floor(room.pot / 2));
    add = Math.min(add, room.chips[srcIdx]); // 올인 상한
    room.chips[srcIdx] -= add; room.pot += add;
    room.bets[srcIdx] = v;
    const done = Object.keys(room.bets).length;
    const n = room.fileIds.length;
    stEachSheet(room, (s, i) => {
      s.getRange(ST.pot.row, ST.pot.col).setValue(room.pot);
      s.getRange(ST.chip.row, ST.chip.col).setValue(room.chips[i]);
      s.getRange(ST.state.row, ST.state.col).setValue('🎲 베팅 ' + done + ' / ' + n);
    });
    stSyncRoster(room);
    if (done >= n) stShowdown(room);
    return;
  }
}
